...
const [startTime, setStartTime] = useState("");
  ...
  const [activeTab, setActiveTab] = useState("history"); // "history" or "live"

  const handleApply = () => {
    if (startTime && endTime) {
      setFetchParams({ start: startTime, end: endTime });
      setActiveTab("history"); // show history map after applying
    }
  };
...
const [coords, setCoords] = useState({ lat: null, lng: null });
 ...
 // 👈 track changes
  ...
  // ✅ MOS values
  { key: "mos1", label: "Main charge mos", color: "#f97316" },
  { key: "mos2", label: "Main discharge mos", color: "#14b8a6" },
  { key: "mos3", label: "APU charge mos", color: "#8b5cf6" },
  { key: "mos4", label: "APU discharge mos", color: "#e11d48" },

  // { key: "ntc5", label: "NTC 5 (°C)", color: "#f97316" },
  // { key: "ntc6", label: "NTC 6 (°C)", color: "#14b8a6" },
  // { key: "ntc7", label: "NTC 7 (°C)", color: "#8b5cf6" },
  // { key: "ntc8", label: "NTC 8 (°C)", color: "#e11d48" },
];
...
const processedData = Array.isArray(livedatafor)
  ? livedatafor.map((item) => {
      ...
      if (Array.isArray(item.ntc)) {
        ...
        if (mosIndex !== -1) {
          // Extract "MOS=" string and following numeric entries
          ...
          // Also include next few numeric values after "MOS=" if they exist
          for (let i = mosIndex + 1; i < item.ntc.length; i++) {
            const val = parseInt(item.ntc[i]);
            if (!isNaN(val)) mosValues.push(val);
            else break; // stop if not number
          }

          // 🟢 Convert numeric MOS values to ON/OFF
          mosValues = mosValues.map((v) => (v === 1 ? "ON" : v === 0 ? "OFF" : null));
        }
...
/>
   ...
   {activePage === "world" && (
    ...
    <div className="w-full h-screen flex flex-col">
      {/* Tabs */}
      <div className="flex flex-col md:flex-row md:items-center gap-4 p-4 bg-[#0d0d0d]/60 border-b border-white/10">
        {/* Start Time */}
        <div>
          ...
          <input
            ...
            className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none w-full md:w-auto"
          />
        </div>
        ...
        <div className="ml-auto flex gap-2">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg ${
              activeTab === "live"
                ? "bg-gradient-to-tr from-green-500 to-lime-400 text-black"
                : "bg-white/10 text-gray-300 hover:bg-white/20"
            }`}
          >
            Live Tracking
        ...
        {vin ? (
          activeTab === "live" ? (
            ...
            <div className="text-white p-4">
              Select start & end time and click Apply to show trip route.
            </div>
          )
        ) : (
          <div className="text-white p-4">
